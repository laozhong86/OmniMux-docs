---
title: "Qwen Image 3.0 生图"
sidebarTitle: "Qwen Image 3.0 生图"
description: "Qwen Image 3.0 生图 · model `qwen-image-3-0`"
api: "POST https://api.omnimux.ai/v1/images/generations"
---

- 调用 `POST /v1/images/generations`，`model` 为 `qwen-image-3-0`
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `qwen-image-3-0` |
...
| --- | --- |
| 单次参考图上限 | 3 张 |
| 单次参考图下限 | 传 `images` 时至少 1 张；文生图不传 |
| 接受格式 | JPG、JPEG、PNG、BMP、TIFF、WEBP、GIF |
| 单文件大小上限 | 10 MB |
