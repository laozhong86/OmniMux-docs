---
title: "Midjourney 生图"
sidebarTitle: "Midjourney 生图"
description: "Midjourney 生图 · model `midjourney`"
api: "POST https://api.omnimux.ai/v1/images/generations"
---

- 调用 `POST /v1/images/generations`，`model` 为 `midjourney`
...
| 项目 | 值 |
| --- | --- |
| 单次参考图上限 | 官方未公布 —— 官方只说明可一次使用多张参考图 |
| 单次参考图下限 | 无 |
| 接受格式 | `.png`、`.gif`、`.webp`、`.jpg`、`.jpeg` |
| 单文件大小上限 | 官方未公布 |

版本在提示词中用 `--v` 选择；已有档案的两个版本 `mj-v7` 与 `mj-v8-1` 公布的参考图格式口径一致。
