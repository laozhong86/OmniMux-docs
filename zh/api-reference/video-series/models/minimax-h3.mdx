...
sidebarTitle: "MiniMax H3 视频生成"
description: "MiniMax H3 视频生成 · model `minimax-h3`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- 创建：`POST /v1/video/generations`，`model` 可选 `minimax-h3`（标准）、`minimax-h3-task`（按次任务通道）、`minimax-h3-turbo`（极速版）或 `minimax-h3-video`（高画质视频专线）
