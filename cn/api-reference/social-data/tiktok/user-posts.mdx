...
- `model` 为 `tiktok-posts`；`messages` 可传 dummy（如 `"."`）
- 业务参数放在请求体 **top-level**（如 `unique_id`），不要塞进 system prompt
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `tiktok-posts` |
| `messages` | array | 是 | 可传 dummy user message |
| `unique_id` | string | 是 | TikTok 用户 unique_id |
