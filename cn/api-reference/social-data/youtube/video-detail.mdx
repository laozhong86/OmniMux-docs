...
- `model` 为 `youtube-video`；`messages` 可传 dummy（如 `"."`）
- 业务参数放在请求体 **top-level**（如 `video_id`），不要塞进 system prompt
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `youtube-video` |
| `messages` | array | 是 | 可传 dummy user message |
| `video_id` | string | 是 | YouTube video id |
