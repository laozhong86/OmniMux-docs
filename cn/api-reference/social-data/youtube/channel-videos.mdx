...
title: "频道视频列表"
sidebarTitle: "频道视频列表"
description: "社交数据 · YouTube · `youtube-posts`"
api: "POST https://api.omnimux.ai/v1/chat/completions"
---

- 社交数据 **读取** 能力：OpenAI Chat Completions 形态，鉴权 `sk-`
- `model` 为 `youtube-posts`；`messages` 可传 dummy（如 `"."`）
- 业务参数放在请求体 **top-level**（如 `channel_id`），不要塞进 system prompt
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `youtube-posts` |
| `messages` | array | 是 | 可传 dummy user message |
| `channel_id` | string | 是 | YouTube channel id |
...
错误形态见右栏示例与 [错误码](/cn/faqs/connection-usage)。
