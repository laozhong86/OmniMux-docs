...
- `model` 为 `youtube-search`；`messages` 可传 dummy（如 `"."`）
- 业务参数放在请求体 **top-level**（如 `search_query`），不要塞进 system prompt
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `youtube-search` |
| `messages` | array | 是 | 可传 dummy user message |
| `search_query` | string | 是 | 搜索词 |
