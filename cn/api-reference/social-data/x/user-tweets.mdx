...
title: "用户推文列表"
sidebarTitle: "用户推文列表"
description: "社交数据 · X · `x-posts`"
api: "POST https://api.omnimux.ai/v1/chat/completions"
---

- 社交数据 **读取** 能力：OpenAI Chat Completions 形态，鉴权 `sk-`
- `model` 为 `x-posts`；`messages` 可传 dummy（如 `"."`）
- 业务参数放在请求体 **top-level**（如 `screen_name`），不要塞进 system prompt
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `x-posts` |
| `messages` | array | 是 | 可传 dummy user message |
| `screen_name` | string | 是 | X 用户 screen_name |
