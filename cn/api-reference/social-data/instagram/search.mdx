...
title: "综合搜索"
sidebarTitle: "综合搜索"
description: "社交数据 · Instagram · `instagram-search`"
api: "POST https://api.omnimux.ai/v1/chat/completions"
---

- 社交数据 **读取** 能力：OpenAI Chat Completions 形态，鉴权 `sk-`
- `model` 为 `instagram-search`；`messages` 可传 dummy（如 `"."`）
- 业务参数放在请求体 **top-level**（如 `query`），不要塞进 system prompt
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `instagram-search` |
...
{
  "error": {
    "message": "Invalid request: missing query or invalid parameter",
