...
title: "用户作品列表"
sidebarTitle: "用户作品列表"
description: "社交数据 · Instagram · `instagram-posts`"
api: "POST https://api.omnimux.ai/v1/chat/completions"
---

- 社交数据 **读取** 能力：OpenAI Chat Completions 形态，鉴权 `sk-`
- `model` 为 `instagram-posts`；`messages` 可传 dummy（如 `"."`）
- 业务参数放在请求体 **top-level**（如 `username`），不要塞进 system prompt
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | 固定为 `instagram-posts` |
...
{
  "error": {
    "message": "Invalid request: missing username or invalid parameter",
