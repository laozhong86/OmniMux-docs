...
| **Social Ops（社媒）** | `GET` / `POST` / `DELETE` | `/api/social/v1/...` | 绑号、媒体、发帖；**非** `/v1` 网关，见 [Social Ops](/cn/api-reference/social-ops) |

左侧「接口参考」中的 AI 端点由 `openapi/relay.json` 生成。Social Ops 为独立用户 API 文档页，不经过 OpenAPI 中继文件。
...
- [Social Ops（社媒绑号 / 发帖）](/cn/api-reference/social-ops)  
- [快速开始](/cn/quickstart)  
