...
- `model` is `instagram-search`; `messages` may be a dummy (e.g. `"."`)
- Business params are **top-level** body fields (e.g. `query`)
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `instagram-search` |
| `messages` | array | yes | Dummy user message allowed |
| `query` | string | yes | Required business field for this capability (see request example) |
