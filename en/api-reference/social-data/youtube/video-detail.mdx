...
- `model` is `youtube-video`; `messages` may be a dummy (e.g. `"."`)
- Business params are **top-level** body fields (e.g. `video_id`)
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `youtube-video` |
| `messages` | array | yes | Dummy user message allowed |
| `video_id` | string | yes | Required business field for this capability (see request example) |
