...
- `model` is `youtube-posts`; `messages` may be a dummy (e.g. `"."`)
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `youtube-posts` |
| `messages` | array | yes | Dummy user message allowed |
| `channel_id` | string | yes | Required business field for this capability (see request example) |
