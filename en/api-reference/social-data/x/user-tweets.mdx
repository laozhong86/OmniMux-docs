...
- `model` is `x-posts`; `messages` may be a dummy (e.g. `"."`)
- Business params are **top-level** body fields (e.g. `screen_name`)
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `x-posts` |
| `messages` | array | yes | Dummy user message allowed |
| `screen_name` | string | yes | Required business field for this capability (see request example) |
