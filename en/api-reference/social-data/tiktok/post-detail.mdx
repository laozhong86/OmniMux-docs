...
- `model` is `tiktok-video`; `messages` may be a dummy (e.g. `"."`)
- Business params are **top-level** body fields (e.g. `aweme_id`)
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `tiktok-video` |
| `messages` | array | yes | Dummy user message allowed |
| `aweme_id` | string | yes | Required business field for this capability (see request example) |
