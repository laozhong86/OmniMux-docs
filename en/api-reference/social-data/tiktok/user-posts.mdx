...
- `model` is `tiktok-posts`; `messages` may be a dummy (e.g. `"."`)
- Business params are **top-level** body fields (e.g. `unique_id`)
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `tiktok-posts` |
| `messages` | array | yes | Dummy user message allowed |
| `unique_id` | string | yes | Required business field for this capability (see request example) |
