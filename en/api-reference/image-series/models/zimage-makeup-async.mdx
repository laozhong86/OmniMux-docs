---
title: "Z Image Generation · Async"
sidebarTitle: "Z Image Generation · Async"
description: "Z Image Generation · Async · model `zimage-makeup-async`"
api: "POST https://api.omnimux.ai/v1/images/generations"
---

- Call `POST /v1/images/generations` with `model` `zimage-makeup-async`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `zimage-makeup-async` |
...
```bash curl --request POST \
  ...
  --data '{"model":"zimage-makeup-async","prompt":"a product photo on white background","n":1}'
```
