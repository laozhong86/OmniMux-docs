...
description: "Nano Banana 2 Image Generation · model `nano-banana-2`"
api: "POST https://api.omnimux.ai/v1/images/generations"
---

- Call `POST /v1/images/generations` with `model` `nano-banana-2`
...
```bash curl --request POST \
  ...
  --data '{"model":"nano-banana-2","prompt":"a product photo on white background","n":1}'
```
