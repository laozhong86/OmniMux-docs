...
sidebarTitle: "Seedream 5.0 Pro Image Generation"
description: "Seedream 5.0 Pro Image Generation · model `seedream-5-0-pro`"
api: "POST https://api.omnimux.ai/v1/images/generations"
---

- Call `POST /v1/images/generations` with `model` `seedream-5-0-pro`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `seedream-5-0-pro` |
...
| Min images per request | 2 for multi-image input, 1 for single-image input; omit `images` for text-to-image |
| Accepted formats | not published by the vendor |
...
```bash curl --request POST \
  ...
  --data '{"model":"seedream-5-0-pro","prompt":"a product photo on white background","n":1}'
```
