---
title: "Qwen Image 3.0 Image Generation"
sidebarTitle: "Qwen Image 3.0 Image Generation"
description: "Qwen Image 3.0 Image Generation · model `qwen-image-3-0`"
api: "POST https://api.omnimux.ai/v1/images/generations"
---

- Call `POST /v1/images/generations` with `model` `qwen-image-3-0`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `qwen-image-3-0` |
...
| Min images per request | 1 when `images` is sent; omit `images` for text-to-image |
| Accepted formats | JPG, JPEG, PNG, BMP, TIFF, WEBP, GIF |
...
```bash curl --request POST \
  ...
  --data '{"model":"qwen-image-3-0","prompt":"a product photo on white background","n":1}'
```
