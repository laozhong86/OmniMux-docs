...
sidebarTitle: "MiniMax H3 Video Generation"
description: "MiniMax H3 Video Generation · model `minimax-h3`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `minimax-h3` (standard), `minimax-h3-task` (fixed per-call task path), `minimax-h3-turbo` (turbo), or `minimax-h3-video` (dedicated video path)
...
| Series | Video series |
| Brand | MiniMax |
| model | `minimax-h3`, `minimax-h3-task`, `minimax-h3-turbo`, `minimax-h3-video` |
...
```bash curl --request POST \
  ...
  --data '{"model":"minimax-h3","prompt":"slow pan across a product on a table"}'
```
