...
sidebarTitle: "Wan 3.0 Video Generation"
description: "Wan 3.0 Video Generation · model `wan-3.0`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `wan-3.0`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `wan-3.0` |
...
```bash curl --request POST \
  ...
  --data '{"model":"wan-3.0","prompt":"slow pan across a product on a table"}'
```
