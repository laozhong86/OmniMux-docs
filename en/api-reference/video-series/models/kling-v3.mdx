...
sidebarTitle: "Kling V3 Video Generation"
description: "Kling V3 Video Generation · model `kling-v3`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `kling-v3`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `kling-v3` |
...
```bash curl --request POST \
  ...
  --data '{"model":"kling-v3","prompt":"slow pan across a product on a table"}'
```
