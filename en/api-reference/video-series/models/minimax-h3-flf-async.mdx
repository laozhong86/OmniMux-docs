---
title: "MiniMax first-last frame async"
sidebarTitle: "MiniMax first-last frame async"
description: "MiniMax first-last frame async · model `minimax-h3-flf-async`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `minimax-h3-flf-async`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `minimax-h3-flf-async` |
...
```bash curl --request POST \
  ...
  --data '{"model":"minimax-h3-flf-async","prompt":"slow pan across a product"}'
```
