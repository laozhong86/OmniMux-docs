...
sidebarTitle: "Seedance 2.5 Video Generation"
description: "Seedance 2.5 Video Generation · model `seedance-2-5`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `seedance-2-5` (standard) or `seedance-2-5-task` (fixed per-call task path)
...
```bash curl --request POST \
  ...
  --data '{"model":"seedance-2-5","prompt":"slow pan across a product on a table"}'
```
