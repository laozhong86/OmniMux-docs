...
sidebarTitle: "Seedance 2.0 Video Generation"
description: "Seedance 2.0 Video Generation · model `seedance-2-0`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `seedance-2-0` (standard) or `seedance-2-0-task` (fixed per-call task path)
...
| Series | Video series |
| Brand | Seedance |
| model | `seedance-2-0`, `seedance-2-0-task` |
...
```bash curl --request POST \
  ...
  --data '{"model":"seedance-2-0","prompt":"slow pan across a product on a table"}'
```
