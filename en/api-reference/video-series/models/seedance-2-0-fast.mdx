...
sidebarTitle: "Seedance 2.0 Fast Video Generation"
description: "Seedance 2.0 Fast Video Generation · model `seedance-2-0-fast`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `seedance-2-0-fast`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `seedance-2-0-fast` |
...
```bash curl --request POST \
  ...
  --data '{"model":"seedance-2-0-fast","prompt":"slow pan across a product on a table"}'
```
