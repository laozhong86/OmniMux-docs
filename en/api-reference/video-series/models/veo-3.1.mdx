...
description: "Veo 3.1 Video Generation · model `veo-3.1`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `veo-3.1`
...
| --- | --- | --- | --- |
| `model` | string | yes | Must be `veo-3.1` |
...
```bash curl --request POST \
  ...
  --data '{"model":"veo-3.1","prompt":"slow pan across a product on a table"}'
```
