...
sidebarTitle: "Grok Imagine Video Generation"
description: "Grok Imagine Video Generation · model `grok-imagine-video`"
api: "POST https://api.omnimux.ai/v1/video/generations"
---

- Create: `POST /v1/video/generations` with `model` `grok-imagine-video`
...
```bash curl --request POST \
  ...
  --data '{"model":"grok-imagine-video","prompt":"slow pan across a product on a table"}'
```
