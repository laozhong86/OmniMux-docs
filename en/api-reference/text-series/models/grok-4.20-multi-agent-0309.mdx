...
````yaml openapi/ops/chat/grok-4.20-multi-agent-0309.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: grok-4.20-multi-agent-0309 · Chat Completions
  description: OpenAI-compatible Chat Completions for `grok-4.20-multi-agent-0309` (Grok) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: grok-4.20-multi-agent-0309 Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `grok-4.20-multi-agent-0309`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_grok_4_20_multi_agent_0309
      requestBody:
        required: true
        content:
          application/json:
...
components:
  securitySchemes:
    BearerAuth:
      type: http
      scheme: bearer
      description: 'API key auth. Header: `Authorization: Bearer sk-...` (create keys in the OmniMux console).'
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `grok-4.20-multi-agent-0309`.
          enum:
          - grok-4.20-multi-agent-0309
          default: grok-4.20-multi-agent-0309
          example: grok-4.20-multi-agent-0309
        messages:
          type: array
          items:
