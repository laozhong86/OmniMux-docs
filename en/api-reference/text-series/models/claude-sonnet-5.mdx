...
description: "Claude · model `claude-sonnet-5` · Chat Completions (Complete)"
api: "POST https://api.omnimux.ai/v1/chat/completions"
---

> - OpenAI Chat Completions compatible
> - Select this page's model via body `model` (`claude-sonnet-5`)
...
````yaml openapi/ops/chat/claude-sonnet-5.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: claude-sonnet-5 · Chat Completions
  description: OpenAI-compatible Chat Completions for `claude-sonnet-5` (Claude) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: claude-sonnet-5 Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `claude-sonnet-5`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_claude_sonnet_5
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `claude-sonnet-5`.
          enum:
          - claude-sonnet-5
