...
````yaml openapi/ops/chat/claude-opus-4-6.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: claude-opus-4-6 · Chat Completions
  description: OpenAI-compatible Chat Completions for `claude-opus-4-6` (Claude) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: claude-opus-4-6 Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `claude-opus-4-6`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_claude_opus_4_6
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `claude-opus-4-6`.
          enum:
          - claude-opus-4-6
