...
````yaml openapi/ops/chat/grok-composer-2.5-fast.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: grok-composer-2.5-fast · Chat Completions
  description: OpenAI-compatible Chat Completions for `grok-composer-2.5-fast` (Grok) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: grok-composer-2.5-fast Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `grok-composer-2.5-fast`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_grok_composer_2_5_fast
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `grok-composer-2.5-fast`.
          enum:
          - grok-composer-2.5-fast
          default: grok-composer-2.5-fast
          example: grok-composer-2.5-fast
        messages:
          type: array
          items:
