...
````yaml openapi/ops/chat/kimi-k3.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: kimi-k3 · Chat Completions
  description: OpenAI-compatible Chat Completions for `kimi-k3` (Kimi) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: kimi-k3 Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `kimi-k3`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_kimi_k3
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `kimi-k3`.
          enum:
          - kimi-k3
          default: kimi-k3
          example: kimi-k3
