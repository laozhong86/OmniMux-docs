...
````yaml openapi/ops/chat/grok-build-0.1.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: grok-build-0.1 · Chat Completions
  description: OpenAI-compatible Chat Completions for `grok-build-0.1` (Grok) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: grok-build-0.1 Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `grok-build-0.1`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_grok_build_0_1
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `grok-build-0.1`.
          enum:
          - grok-build-0.1
