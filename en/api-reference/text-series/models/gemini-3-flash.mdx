...
description: "Gemini · model `gemini-3-flash` · Chat Completions (Complete)"
api: "POST https://api.omnimux.ai/v1/chat/completions"
---

> - OpenAI Chat Completions compatible
> - Select this page's model via body `model` (`gemini-3-flash`)
...
````yaml openapi/ops/chat/gemini-3-flash.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: gemini-3-flash · Chat Completions
  description: OpenAI-compatible Chat Completions for `gemini-3-flash` (Gemini) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: gemini-3-flash Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `gemini-3-flash`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_gemini_3_flash
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `gemini-3-flash`.
          enum:
          - gemini-3-flash
