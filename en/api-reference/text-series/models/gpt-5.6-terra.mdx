...
````yaml openapi/ops/chat/gpt-5.6-terra.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: gpt-5.6-terra · Chat Completions
  description: OpenAI-compatible Chat Completions for `gpt-5.6-terra` (GPT) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: gpt-5.6-terra Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `gpt-5.6-terra`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_gpt_5_6_terra
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `gpt-5.6-terra`.
          enum:
          - gpt-5.6-terra
