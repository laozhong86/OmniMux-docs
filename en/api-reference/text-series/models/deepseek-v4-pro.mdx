...
````yaml openapi/ops/chat/deepseek-v4-pro.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: deepseek-v4-pro · Chat Completions
  description: OpenAI-compatible Chat Completions for `deepseek-v4-pro` (DeepSeek) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: deepseek-v4-pro Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `deepseek-v4-pro`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_deepseek_v4_pro
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `deepseek-v4-pro`.
          enum:
          - deepseek-v4-pro
