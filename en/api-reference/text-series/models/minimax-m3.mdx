...
````yaml openapi/ops/chat/minimax-m3.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: minimax-m3 · Chat Completions
  description: OpenAI-compatible Chat Completions for `minimax-m3` (MiniMax) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: minimax-m3 Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `minimax-m3`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_minimax_m3
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `minimax-m3`.
          enum:
          - minimax-m3
          default: minimax-m3
          example: minimax-m3
