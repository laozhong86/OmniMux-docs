...
````yaml openapi/ops/chat/deepseek-v4-flash.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: deepseek-v4-flash · Chat Completions
  description: OpenAI-compatible Chat Completions for `deepseek-v4-flash` (DeepSeek) on OmniMux gateway.
  version: 1.0.0
servers:
  ...
  /v1/chat/completions:
    post:
      ...
      description: '- OpenAI Chat Completions protocol
        ...
        - Select model via body `model` = `deepseek-v4-flash`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_deepseek_v4_flash
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `deepseek-v4-flash`.
          enum:
          - deepseek-v4-flash
