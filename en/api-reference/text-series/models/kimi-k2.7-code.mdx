...
````yaml openapi/ops/chat/kimi-k2.7-code.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: kimi-k2.7-code · Chat Completions
  description: OpenAI-compatible Chat Completions for `kimi-k2.7-code` (Kimi) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: kimi-k2.7-code Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `kimi-k2.7-code`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_kimi_k2_7_code
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `kimi-k2.7-code`.
          enum:
          - kimi-k2.7-code
