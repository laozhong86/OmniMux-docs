...
````yaml openapi/ops/chat/grok-4.20-0309-reasoning.json POST /v1/chat/completions
openapi: 3.1.0
info:
  title: grok-4.20-0309-reasoning · Chat Completions
  description: OpenAI-compatible Chat Completions for `grok-4.20-0309-reasoning` (Grok) on OmniMux gateway.
  version: 1.0.0
servers:
...
paths:
  /v1/chat/completions:
    post:
      tags:
      - Chat Completion
      summary: grok-4.20-0309-reasoning Chat Completions
      description: '- OpenAI Chat Completions protocol

        - Select model via body `model` = `grok-4.20-0309-reasoning`

        - Synchronous by default; set `stream: true` for SSE

        - Multimodal / tools fields per gateway schema when supported'
      operationId: createChatCompletion_grok_4_20_0309_reasoning
      requestBody:
        required: true
        content:
          application/json:
  ...
  schemas:
    ChatCompletionRequest:
      ...
      properties:
        model:
          type: string
          description: Model id. This page pins `grok-4.20-0309-reasoning`.
          enum:
