...
for lang in ("en", "zh"):
    ...
    op = {"summary": title, "operationId": "generateGPTImage", "description": description, "security": [{"BearerAuth": []}], "requestBody": {"required": True, "content": {"application/json": {"schema": {"type": "object", "required": ["model", "prompt"], "properties": {
        ...
        "quality": {"type": "string", "description": "质量选项取决于所选模型与渠道。" if zh else "Quality options depend on the selected model and route."},
        "images": {"type": "array", "items": {"type": "string"}, "description": "参考图：图片 URL 或 base64。image / image_urls / input_reference 为可接受的别名，网关归一为 images。" if zh else "Reference images: image URL or base64. image / image_urls / input_reference are accepted aliases and are normalized to images."}}}, "example": example}}},
